...
class CryptoManager {
    ...
    private fun getKey(): SecretKey {
        val existingKey: SecretKeyEntry? = keyStore.getEntry("secret", null) as? SecretKeyEntry
    ...
    fun decrypt(inputStream: InputStream): ByteArray {
        return inputStream.use {
            val ivSize: Int = inputStream.read()
            val iv: ByteArray = ByteArray(ivSize)
            it.read(iv)

            val encryptedBytesSize: Int = inputStream.read()
            val encryptedBytes: ByteArray = ByteArray(encryptedBytesSize)
            it.read()
